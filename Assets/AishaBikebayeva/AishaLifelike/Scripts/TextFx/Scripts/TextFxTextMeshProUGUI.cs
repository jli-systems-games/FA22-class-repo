		...
		protected override void GenerateTextMesh()
		{
			...
			else
			{
				...
				for(int idx=0; idx < m_textInfo.meshInfo.Length; idx++)
				{
					...
					CanvasRenderer targetCanvasRenderer = idx == 0 ? m_canvasRendererComp : m_subTextObjects[idx].canvasRenderer;

					targetMesh.vertices = meshVertsToUse[idx];
					targetMesh.colors32 = meshColsToUse[idx];

					if(targetCanvasRenderer != null)
					{
						targetCanvasRenderer.SetMesh(targetMesh);
					}
				}
